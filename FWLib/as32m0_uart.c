...
//	 uint32_t BaudIntDiv;
//	 
//	 float BaudRateDiv = (ClockFrequency >> 4) / BaudRate;
//	 
//   /* Calculate integer baud rate register value */
//   BaudIntDiv = (uint32_t) BaudRateDiv;
//
//   
...
//   {
//   	   io_write(BB_SRAM_BASE, 0xdeadface);  // debug
//   }
//   
...
//       pBase->FractBaudDivisor = (uint32_t) ((BaudRateDiv * 64) + 0.5);
//   } 
//   
//   	//--------------------------
//    // USER NOTE :
...
//  Update ther previous function for Cortex-M4 not support hard float calculate
void apUART_BaudRateSet(UART_TypeDef* pBase, uint32_t ClockFrequency, uint32_t BaudRate)
{
...
{
	pBase->Control |= 1<<bsUART_TRANSMIT_ENABLE; 
}
...
{
	pBase->Control |= 1<<bsUART_RECEIVE_ENABLE; 
}
...
{
	pBase->IntClear |= UART_INTCLR_TRANSMIT;
}
void apUART_INT_RX_CLR(UART_TypeDef* pBase)
{
	pBase->IntClear |= UART_INTCLR_RECEIVE;
}

void apUART_Set_TXFIFO_WATERLEVEL(UART_TypeDef* pBase, uint16_t water_level)
{
	pBase->FifoSelect &= ~(0x111<<bsUART_TRANS_INT_LEVEL);
	pBase->FifoSelect |= (water_level<< bsUART_TRANS_INT_LEVEL);
}
void apUART_Set_RXFIFO_WATERLEVEL(UART_TypeDef* pBase, uint16_t water_level)
{
	pBase->FifoSelect &= ~(0x111<<bsUART_RECV_INT_LEVEL);
	pBase->FifoSelect |= (water_level<< bsUART_RECV_INT_LEVEL);
}
...
void apUART_Initialize (UART_TypeDef* pBase, UART_sStateStruct* UARTx)
{
  ...
  pBase->FifoSelect = (UARTx->txfifo_watchlevel << bsUART_TRANS_INT_LEVEL ) | // TXIFLSEL
...
void apUART_DMAModeSet(UART_TypeDef* pBase,UART_eDMAMode DMAMode)
{
	...
	{
		case UART_DMA_TX_ON:
			pBase->DmaCon |= (1<<bsUART_DMA_TRANSMIT_ENABLE);
			break;
		case UART_DMA_TX_OFF:
			pBase->DmaCon &= ~(1<<bsUART_DMA_TRANSMIT_ENABLE);
			break;
		case UART_DMA_RX_ON:
			pBase->DmaCon |= (1<<bsUART_DMA_RECEIVE_ENABLE);
			break;
		case UART_DMA_RX_OFF:
			pBase->DmaCon &= ~(1<<bsUART_DMA_RECEIVE_ENABLE);
			break;
	}
}
...
{
		pBase->Control &= ~(1<<bsUART_ENABLE);
		while(apUART_Check_BUSY(pBase)==1);
	if(LoopMode != DISABLE)
	{	
		pBase->Control |= (1<< bsUART_LOOP_BACK);
	}
	else
	{	 
		pBase->Control &= ~(1<< bsUART_LOOP_BACK);
