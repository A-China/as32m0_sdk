...
void ssp_config(uint8_t div_clk)
{
	...
	/* Set Device Parameters */
 	...
 	pParam_ptr->ClockPrescale    = div_clk;  //8MHz
...
void  ssp_slave_config(void)
{
      ...
      pParam_ptr->eDataSize        = apSSP_DATASIZE_8BITS;
      pParam_ptr->eLoopBackMode    = apSSP_LOOPBACKOFF;
      pParam_ptr->eMasterSlaveMode = apSSP_SLAVE;
      pParam_ptr->eSlaveOutput     = apSSP_SLAVEOUTPUTENABLED;
      apSSP_DeviceParametersSet(APB_SPI, pParam_ptr); 
} 

void ssp_send_byte(uint8_t data){
